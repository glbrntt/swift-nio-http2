        ...
        case .accumulatingFrameHeader(var state):
            ...
            if header.type != 0 {
                ...
                if expectedPadding == remainingBytes {
                    ...
                    let dataPayload = HTTP2Frame.FramePayload.Data(data: .byteBuffer(allocator.buffer(capacity: 0)), endStream: header.flags.contains(.endStream), paddingBytes: nil)
        ...
        case .accumulatingData(var state):
            if state.header.type == 0 && state.accumulatedBytes.readableBytes > 0 {
                ...
                if expectedPadding == remainingBytes {
                    ...
                    let dataPayload = HTTP2Frame.FramePayload.Data(data: .byteBuffer(allocator.buffer(capacity: 0)), endStream: state.header.flags.contains(.endStream), paddingBytes: nil)
                    let outputFrame = HTTP2Frame(streamID: streamID, payload: .data(dataPayload))
